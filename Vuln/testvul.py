...
class HackerToolboxApp(App):
    ...
    def build(self):
        ...
        gif_image = AsyncImage(source='test.gif')
        ...
        self.ip_entry = TextInput(size_hint=(None, None), size=(200, 30), pos_hint={'center_x': 0.5})
        ...
        ssh_button = Button(text="Connexion SSH", on_press=self.ssh_connect)
    ...
    def discover_network(self, instance):
        ...
        self.report_results['Hosts découverts'] = len(hosts)
    ...
    def scan_ports(self, instance):
        ...
        for host in nm.all_hosts():
            for proto in nm[host].all_protocols():
                ...
                self.report_results['Ports ouverts'] += len(ports)
    ...
    def detect_vulnerabilities(self, instance):
        ...
        self.host_ip = ip_address
        nm = nmap.PortScanner()
        nm.scan(hosts=ip_address, arguments='--script vulners.nse')

        vulnerabilities = nm[ip_address]['tcp'].get('script', {}).get('vulners', None)
        if vulnerabilities:
            self.report_results['Vulnérabilités'] += 1

        popup = Popup(title="Résultats de la détection de vulnérabilités", content=Label(text="Vulnérabilités détectées"), size_hint=(None, None), size=(400, 300))
        popup.open()

    def generate_report(self, instance):
        categories = list(self.report_results.keys())
        data = list(self.report_results.values())
        ...
        logo_img = ReportImage("hack.png", width=100, height=100)  # Chemin vers votre image PNG
        ...
        report_content.append(Paragraph(str(self.report_results), self.styles['Normal']))
    ...
    def ssh_connect(self, instance):
        ...
        self.report_results['Connexions SSH'] = "\n".join(login_attempts)
